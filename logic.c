...
int check_order_complete(elev_motor_direction_t dir){
	...
	if (current_floor != -1) {
		elev_set_floor_indicator(current_floor);
		if (dir == DIRN_UP ) {
			if (get_order(current_floor, BUTTON_COMMAND) || get_order(current_floor, BUTTON_CALL_UP) || order_is_in_dir(dir) == -1) {
				return 1;
			}
		}else if (dir == DIRN_DOWN) {
			if (get_order(current_floor, BUTTON_COMMAND) || get_order(current_floor, BUTTON_CALL_DOWN) || order_is_in_dir(dir) == -1) {
